...
from flcore.servers.servercos import FedCos
from flcore.servers.serverours import FedDRL
from flcore.servers.serverdrl_head import FedDRLHEAD
from flcore.servers.serverdrl_body import FedDRLBODY
from flcore.servers.serverdrl_fullper import FedDRLFULLPER
from flcore.servers.serverdrl_fullwei import FedDRLFULLWEI
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if args.algorithm == "FedAvg":
        ...
        elif args.algorithm == "FedCos":
            server = FedCos(args, i)

        elif args.algorithm == "APPLE":
            server = APPLE(args, i)

        elif args.algorithm == "FedDRL":
            # args.predictor = copy.deepcopy(args.model.fc)
            # args.model.fc = nn.Identity()
            # args.model = LocalModel(args.model, args.predictor)
            server = FedDRL(args, i)
        elif args.algorithm == "FedDRLHEAD": # equal fedDRLAVG
            server = FedDRLHEAD(args, i)
        elif args.algorithm == "FedDRLBODY":
            server = FedDRLBODY(args, i)
        elif args.algorithm == "FedDRLFULLPER":
            server = FedDRLFULLPER(args, i)
        elif args.algorithm == "FedDRLFULLWEI":
            server = FedDRLFULLWEI(args, i)
            ...
            # args.predictor = copy.deepcopy(nn.Sequential(
            #     args.model.fc1, 
            #     args.model.fc
            # ))
    ...
    # args.algorithm = args.algorithm + '_frac' + str(args.join_ratio) + '_l' +str(args.lamda) + '_mu' + str(args.mu)
    # args.algorithm = args.algorithm + '_l' +str(args.lamda) + '_mu' + str(args.mu)
    # # Global average
...
if __name__ == "__main__":
    ...
    parser.add_argument('-seed', "--random_seed", type=int, default='2024')
    # practical
    ...
    # FedDRL
    parser.add_argument('-lrt', "--lr_step", type=int, default=500,
