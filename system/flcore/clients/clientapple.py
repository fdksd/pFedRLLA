...
class clientAPPLE(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        self.num_clients = int(args.num_clients * args.join_ratio)
        self.lamda = 1
        self.mu = args.mu
    ...
    def train(self, R):
        ...
        for step in range(max_local_steps):
            for i, (x, y) in enumerate(trainloader):
                ...
                tmp_val = (np.array(self.ps)-np.array(self.p0)).sum()
                for param in self.model.parameters():
                    param.grad = param.grad + self.lamda * self.mu * tmp_val
                self.optimizer.step()
