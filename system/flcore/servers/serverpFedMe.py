...
class pFedMe(Server):
    ...
    def save_results(self):
        ...
        if (len(self.rs_test_acc_per)):
            ...
            file_path = result_path + "{}.csv".format(algo2)
            print("===============================\n", file_path)

            data = []
            cols = ['rs_test_acc', 'rs_train_acc', 'rs_train_loss']
            for idx, test_acc in enumerate(self.rs_test_acc_per):
                data.append([test_acc, self.rs_train_acc_per[idx], self.rs_train_loss_per[idx]])
